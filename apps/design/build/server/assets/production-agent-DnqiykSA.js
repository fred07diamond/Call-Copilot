...
<selection>\n${sel.text}\n</selection>`;
		...
		const filesContextPromise = (async () => {
			...
			if (history.length === 0) try {
				const { resourceListAccessible, SHARED_OWNER, resourceGet } = await import("./store-C4t59_TO.js");
		...
		startRun(runId, threadId ?? runId, async (send, signal) => {
			...
			if (agentRefs.length > 0 && requestMode !== "plan") {
				const [{ A2AClient, callAgent }, { resolveA2ACallerAuth }] = await Promise.all([import("./client-1j91N6-z.js").then((n) => n.i), import("./caller-auth-OyZElvKs.js")]);
