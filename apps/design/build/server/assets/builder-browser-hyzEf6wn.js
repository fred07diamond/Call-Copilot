import { b as getAuthSecret, d as getAppBasePath, p as getOrigin } from "./auth-B6XASyqO.js";
import { createHmac, randomBytes, timingSafeEqual } from "node:crypto";
//#region ../../node_modules/.pnpm/@agent-native+core@0.14.8_bd141c93c3ba8c1834a821745ffc16c2/node_modules/@agent-native/core/dist/server/builder-browser.js
var DEFAULT_BUILDER_APP_HOST = "https://builder.io";
var DEFAULT_BUILDER_API_HOST = "https://api.builder.io";
var BUILDER_BROWSER_HOST = "agent-native-browser";
var BUILDER_BROWSER_CLIENT_ID = "Agent Native Browser";
var BUILDER_CALLBACK_PATH = "/_agent-native/builder/callback";
/**
...
*/
var BUILDER_STATE_PARAM = "_an_state";
var BUILDER_CONNECT_PARAM = "_an_connect";
var BUILDER_STATE_TTL_MS = 600 * 1e3;
function signingKeyForPurpose(purpose) {
	return purpose === "callback" ? `builder-csrf:${getAuthSecret()}` : `builder-connect:${getAuthSecret()}`;
}
function macForParts(purpose, nonce, emailEncoded, ts) {
	return createHmac("sha256", signingKeyForPurpose(purpose)).update(`${nonce}.${emailEncoded}.${ts}`).digest("base64url");
}
function signEmailBoundBuilderToken(ownerEmail, purpose) {
	const nonce = randomBytes(16).toString("base64url");
	const ts = Date.now();
	const emailEncoded = Buffer.from(ownerEmail, "utf8").toString("base64url");
	return `${nonce}.${emailEncoded}.${ts}.${macForParts(purpose, nonce, emailEncoded, ts)}`;
}
function verifyEmailBoundBuilderToken(token, ownerEmail, purpose) {
...
function signBuilderConnectToken(ownerEmail) {
	return signEmailBoundBuilderToken(ownerEmail, "connect");
}
function verifyBuilderConnectToken(token, ownerEmail) {
	return verifyEmailBoundBuilderToken(token, ownerEmail, "connect");
}
function appendBuilderConnectToken(connectUrl, ownerEmail) {
	const url = new URL(connectUrl);
	url.searchParams.set(BUILDER_CONNECT_PARAM, signBuilderConnectToken(ownerEmail));
	return url.toString();
}
function isAllowedBrowserReturnUrl(urlString) {
	try {
		...
		return isAllowedProtocol && (isLocalhost || isBuilderDomain || isAgentNativeDomain);
	} catch {
		return false;
	}
}
...
function getBuilderApiHost() {
	return process.env.AIR_HOST || process.env.BUILDER_HOST || process.env.BUILDER_API_HOST || DEFAULT_BUILDER_API_HOST;
}
function getConfiguredBuilderBranchProjectId() {
	return (process.env.DISPATCH_BUILDER_PROJECT_ID || process.env.BUILDER_BRANCH_PROJECT_ID || process.env.BUILDER_PROJECT_ID)?.trim() || void 0;
}
function isBuilderBranchingEnabled() {
	return !!getConfiguredBuilderBranchProjectId();
}
async function resolveBuilderBranchProjectId() {
	const envProjectId = getConfiguredBuilderBranchProjectId();
	if (envProjectId) return envProjectId;
	try {
		const { resolveSecret } = await import("./credential-provider-CKFlFM2V.js");
		for (const key of [
...
*/
function buildBuilderCliAuthUrl(origin, state = null) {
	const normalizedOrigin = normalizeOrigin(origin);
	const appBasePath = getAppBasePath();
	const callbackUrl = new URL(`${appBasePath}${BUILDER_CALLBACK_PATH}`, normalizedOrigin);
	if (state) callbackUrl.searchParams.set(BUILDER_STATE_PARAM, state);
...
*/
function getBuilderBrowserConnectUrl(origin) {
	return `${normalizeOrigin(origin)}${getAppBasePath()}/_agent-native/builder/connect`;
}
function getBuilderBrowserStatus(origin) {
	...
	return {
		...
		branchProjectId: branchProjectId || void 0,
		envManaged,
		credentialSource: envManaged ? "env" : void 0,
		...
		userId: process.env.BUILDER_USER_ID || void 0,
		orgName: process.env.BUILDER_ORG_NAME || void 0,
		orgKind: process.env.BUILDER_ORG_KIND || void 0
	};
}
function getBuilderBrowserStatusForEvent(event) {
...
function resolveSafePreviewUrl(previewUrl, event) {
	if (previewUrl && isAllowedBrowserReturnUrl(previewUrl)) return previewUrl;
...
*/
var BUILDER_CALLBACK_THEME_SCRIPT = `<script>
(function () {
  try {
...
`;
function createBuilderBrowserCallbackPage(previewUrl) {
...
*/
function createBuilderBrowserCallbackErrorPage(message) {
	return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function normalizeBuilderApiString(value, fieldName) {
	if (typeof value !== "string" || !value.trim()) throw new Error(`Builder agent run returned a blank ${fieldName}`);
	const trimmed = value.trim();
	if (/[\u0000-\u001f\u007f]/.test(trimmed)) throw new Error(`Builder agent run returned a malformed ${fieldName}`);
	return trimmed;
}
function normalizeBuilderBranchUrl(value) {
	...
	if (parsed.protocol !== "https:" && parsed.protocol !== "http:") throw new Error("Builder agent run returned a malformed url");
	if (parsed.hostname !== "builder.io" && !parsed.hostname.endsWith(".builder.io")) throw new Error("Builder agent run returned a non-Builder url");
...
*/
async function runBuilderAgent(args) {
	const { resolveBuilderCredentials } = await import("./credential-provider-CKFlFM2V.js");
	const creds = await resolveBuilderCredentials();
	if (!creds.privateKey || !creds.publicKey) throw new Error("Builder keys are not configured");
	if (!args.prompt || !args.prompt.trim()) throw new Error("prompt is required");
	const projectId = args.projectId?.trim();
	if (!projectId) throw new Error("Builder project ID is not configured. Set DISPATCH_BUILDER_PROJECT_ID, BUILDER_BRANCH_PROJECT_ID, or BUILDER_PROJECT_ID.");
	const builderUserId = args.userId || creds.userId || void 0;
	const builderUserEmail = builderUserId ? void 0 : args.userEmail;
	if (!builderUserEmail && !builderUserId) throw new Error("userEmail or userId is required");
	...
	if (!response.ok) {
		const msg = typeof parsed.error === "string" ? parsed.error : `Builder agent run failed (${response.status})`;
		throw new Error(msg);
	}
	return {
		branchName: normalizeBuilderApiString(parsed.branchName, "branchName"),
		projectId: typeof parsed.projectId === "string" && parsed.projectId.trim() ? parsed.projectId.trim() : projectId,
		url: normalizeBuilderBranchUrl(parsed.url),
		status: typeof parsed.status === "string" && parsed.status.trim() ? parsed.status.trim() : "processing"
	};
}
async function requestBuilderBrowserConnection(args) {
	const { resolveBuilderCredentials } = await import("./credential-provider-CKFlFM2V.js");
	const creds = await resolveBuilderCredentials();
	if (!creds.privateKey || !creds.publicKey) throw new Error("Builder browser access is not configured");
	...
	const response = await fetch(url, {
		...
		body: JSON.stringify({
			...
			branchName: args.branchName || void 0,
			proxyOrigin: args.proxyOrigin || void 0,
			proxyDefaultOrigin: args.proxyDefaultOrigin || void 0,
			proxyDst: args.proxyDestination || void 0
		})
	});
	const body = await response.json().catch(() => ({}));
	if (!response.ok) {
		const error = typeof body.error === "string" ? body.error : `Builder browser request failed (${response.status})`;
		throw new Error(error);
	}
	return body;
}
//#endregion
export { resolveSafePreviewUrl as _, appendBuilderConnectToken as a, verifyBuilderConnectToken as b, createBuilderBrowserCallbackPage as c, getBuilderBrowserConnectUrl as d, getBuilderBrowserStatus as f, resolveBuilderBranchProjectId as g, requestBuilderBrowserConnection as h, BUILDER_STATE_PARAM as i, getBuilderApiHost as l, isBuilderBranchingEnabled as m, BUILDER_CONNECT_PARAM as n, buildBuilderCliAuthUrl as o, getBuilderBrowserStatusForEvent as p, BUILDER_ENV_KEYS as r, createBuilderBrowserCallbackErrorPage as s, BUILDER_CALLBACK_PATH as t, getBuilderAppHost as u, runBuilderAgent as v, signBuilderConnectToken as y };
