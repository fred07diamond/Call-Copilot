import { i as getDbExec, o as intType, p as retryOnDdlRace, u as isPostgres } from "./client-BnpqLOqs.js";
import { t as isDuplicateColumnError } from "./migrations-Bx6wupZW.js";
//#region ../../node_modules/.pnpm/@agent-native+core@0.14.8_bd141c93c3ba8c1834a821745ffc16c2/node_modules/@agent-native/core/dist/observability/store.js
/**
...
async function ensureObservabilityTables() {
	if (!_initPromise) _initPromise = (async () => {
      ...
      `));
		for (const table of USER_SCOPED_TABLES) try {
...
async function insertTraceSpan(span) {
...
async function upsertTraceSummary(summary) {
	await ensureObservabilityTables();
	const client = getDbExec();
	if (isPostgres()) await client.execute({
...
async function getTraceSpansForRun(runId, opts = {}) {
	await ensureObservabilityTables();
	const client = getDbExec();
...
async function getTraceSummaries(opts) {
	await ensureObservabilityTables();
	const client = getDbExec();
...
async function getTraceSummary(runId, opts = {}) {
	await ensureObservabilityTables();
	const client = getDbExec();
	const { where, args } = withUserFilter("run_id = ?", [runId], opts.userId);
	const { rows } = await client.execute({
...
async function getFeedback(opts) {
	await ensureObservabilityTables();
	const client = getDbExec();
	const conditions = [];
	const args = [];
...
async function getFeedbackStats(sinceMs, opts = {}) {
	await ensureObservabilityTables();
	const client = getDbExec();
	const { where, args } = withUserFilter("created_at >= ?", [sinceMs], opts.userId);
	const { rows } = await client.execute({
	...
	for (const row of rows) {
		...
		else if (row.feedback_type === "category") categories[String(row.value)] = cnt;
	}
	return {
		total,
		thumbsUp,
...
async function upsertSatisfactionScore(score) {
	await ensureObservabilityTables();
	const client = getDbExec();
	if (isPostgres()) await client.execute({
...
async function getSatisfactionScores(opts) {
	await ensureObservabilityTables();
	const client = getDbExec();
	const conditions = [];
	const args = [];
...
async function insertEvalResult(result) {
	await ensureObservabilityTables();
	await getDbExec().execute({
...
async function getEvalsForRun(runId, opts = {}) {
	await ensureObservabilityTables();
	const client = getDbExec();
	const { where, args } = withUserFilter("run_id = ?", [runId], opts.userId);
	const { rows } = await client.execute({
...
async function getEvalStats(sinceMs, opts = {}) {
	await ensureObservabilityTables();
	const client = getDbExec();
	const { where, args } = withUserFilter("created_at >= ?", [sinceMs], opts.userId);
...
async function insertExperiment(exp) {
	await ensureObservabilityTables();
	await getDbExec().execute({
...
async function updateExperiment(id, updates) {
	...
	if (updates.variants !== void 0) {
		sets.push("variants = ?");
		args.push(JSON.stringify(updates.variants));
	}
	if (updates.metrics !== void 0) {
		sets.push("metrics = ?");
...
async function listExperiments() {
	await ensureObservabilityTables();
	const { rows } = await getDbExec().execute(`SELECT * FROM agent_experiments ORDER BY created_at DESC`);
...
async function insertExperimentResult(result) {
	await ensureObservabilityTables();
	await getDbExec().execute({
...
async function getObservabilityOverview(sinceMs, opts = {}) {
	await ensureObservabilityTables();
	const client = getDbExec();
...
function rowToExperiment(row) {
	return {
		...
		createdAt: Number(row.created_at),
		ownerEmail: typeof row.owner_email === "string" && row.owner_email ? row.owner_email : null
	};
}
...
//#endregion
export { ensureObservabilityTables, getAssignment, getEvalStats, getEvalsForRun, getExperiment, getExperimentResults, getFeedback, getFeedbackStats, getObservabilityOverview, getSatisfactionScores, getTraceSpansForRun, getTraceSummaries, getTraceSummary, insertEvalResult, insertExperiment, insertExperimentResult, insertFeedback, insertTraceSpan, listExperiments, updateExperiment, upsertAssignment, upsertSatisfactionScore, upsertTraceSummary };
