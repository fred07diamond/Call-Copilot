...
//#region ../../node_modules/.pnpm/@agent-native+core@0.14.8_bd141c93c3ba8c1834a821745ffc16c2/node_modules/@agent-native/core/dist/scripts/db/query.js
/**
...
function printTable(rows, finalSql, format) {
	...
	for (const row of rows) {
		const line = keys.map((k, i) => {
			...
			return val.length > 60 ? val.slice(0, 57) + "..." : val.padEnd(widths[i]);
		}).join(" | ");
		console.log(line);
	}
}
...
Options:
  ...
  --help          Show this help message`);
	...
	if (!upper.startsWith("SELECT") && !upper.startsWith("WITH") && !upper.startsWith("EXPLAIN") && !upper.startsWith("PRAGMA")) fail("Only SELECT, WITH, EXPLAIN, and PRAGMA queries are allowed. Use db-exec for writes.");
	...
	let finalSql = sql;
	if (parsed.limit && (upper.startsWith("SELECT") || upper.startsWith("WITH")) && !/\bLIMIT\b/i.test(stripped)) {
		const limitVal = parseInt(parsed.limit, 10);
		if (isNaN(limitVal) || limitVal < 1) fail("--limit must be a positive integer");
		...
		try {
			...
			await pgSql.begin(async (tx) => {
				...
				try {
					...
					const result = sqlArgs.length > 0 ? await tx.unsafe(pgSqlText, sqlArgs) : await tx.unsafe(pgSqlText);
	...
	try {
		...
		const result = sqlArgs.length > 0 ? await client.execute({
			sql: finalSql,
			args: sqlArgs
		}) : await client.execute(finalSql);
		printTable(result.rows.map((row) => {
			const obj = {};
			for (let i = 0; i < result.columns.length; i++) obj[result.columns[i]] = row[i];
			return obj;
		}), finalSql, parsed.format);
