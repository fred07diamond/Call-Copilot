...
| ------------ | ----------------------------------------- | -------------------------- |
| `navigation` | Current view (`call-copilot` or `new-app`) | UI -> Agent (read-only)    |
| `navigate`   | Navigate command (one-shot, auto-deleted) | Agent -> UI (auto-deleted) |
| `call-copilot.session` | Live transcript, listening state, matched watch phrases | UI -> Agent (read-only) |
...
This app is mounted at `/call-copilot` in a workspace. Inside app source, React Router paths are app-local: use `<Link to="/">` and `navigate("/")`, not `/call-copilot/`.
...
**Running actions from the frame:** The terminal cwd is the workspace root. Always `cd` to this app's root before running any action:

```bash
cd apps/call-copilot && pnpm action <name> [args]
```
...
| `manage-keywords` | `--action list\|set\|add\|remove` plus `--phrases` or `--phrase` | Maintain watch phrase highlights |
| `quick-call-score` | `--transcriptText <text> [--prospectContext <text>] [--transcriptId <id>]` | Queue pending row + HTTP worker only (no LLM in action) |
| `create-call-analysis` | `--transcriptText <text> [--prospectContext <text>] [--transcriptId <id>]` | Same as \`quick-call-score\` (queue only) |
| `publish-call-analysis` | `--id <analysis-id>` plus structured result fields, or `--status error --error <text>` | Save agent-provided JSON (optional; worker usually fills results) |
| `deep-call-analysis` | `--id <analysis-id>` | Reset row to pending and re-queue worker (no LLM in action) |
