...
//#region ../../node_modules/.pnpm/@agent-native+core@0.14.8_d8cd0b9ea502c97d2f2b6c28b2fc9a81/node_modules/@agent-native/core/dist/server/agent-discovery.js
...
async function discoverAgents(selfAppId) {
	...
	try {
		const { resourceList, resourceGet, SHARED_OWNER } = await import("./store-CjfU7ZK0.js");
		const { parseRemoteAgentManifest, REMOTE_AGENT_RESOURCE_PREFIXES } = await import("./metadata-BxnFNJ7Y.js").then((n) => n.c);
		const resources = [];
...
function findWorkspaceRoot(startDir = process.cwd()) {
	...
	for (let i = 0; i < 20; i++) {
		if (typeof readJson(nodePath.join(dir, "package.json"))?.["agent-native"]?.workspaceCore === "string") return dir;
...
function workspaceAppsManifestCandidates() {
	const candidates = [];
	try {
		candidates.push(nodePath.join(process.cwd(), ".agent-native", WORKSPACE_APPS_MANIFEST_FILE), nodePath.join(process.cwd(), WORKSPACE_APPS_MANIFEST_FILE));
	} catch {}
	try {
		const moduleDir = nodePath.dirname(fileURLToPath(import.meta.url));
		candidates.push(nodePath.join(moduleDir, ".agent-native", WORKSPACE_APPS_MANIFEST_FILE), nodePath.join(moduleDir, WORKSPACE_APPS_MANIFEST_FILE));
	} catch {}
	return candidates;
}
...
function readWorkspaceAppsFromFilesystem() {
	...
	const appsDir = nodePath.join(workspaceRoot, "apps");
	...
	const apps = fs.readdirSync(appsDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => {
		const appDir = nodePath.join(appsDir, entry.name);
		const pkg = readJson(nodePath.join(appDir, "package.json"));
		if (!pkg) return null;
		return {
			id: entry.name,
