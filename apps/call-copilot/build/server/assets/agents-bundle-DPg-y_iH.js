...
//#region ../../node_modules/.pnpm/@agent-native+core@0.14.8_d8cd0b9ea502c97d2f2b6c28b2fc9a81/node_modules/@agent-native/core/dist/server/agents-bundle.js
/**
...
function readSkillsDir(skillsDir, rootForRelative, out, skipExistingNames) {
	...
	for (const entry of entries) {
		...
		const skillDirAbs = nodePath.join(skillsDir, entry.name);
		const skillFile = nodePath.join(skillDirAbs, "SKILL.md");
		try {
			...
			try {
				const walk = (subdir, prefix) => {
					for (const e of fs.readdirSync(subdir, { withFileTypes: true })) {
						const abs = nodePath.join(subdir, e.name);
			...
			out[name] = {
				...
				content,
				dir: nodePath.relative(rootForRelative, skillDirAbs).replace(/\\/g, "/"),
				extraFiles
			};
		} catch {}
	}
...
function readAgentsBundleFromFs(cwd, workspaceSource = null) {
	let agentsMd = "";
	try {
		const agentsMdPath = nodePath.join(cwd, "AGENTS.md");
	...
	const skills = {};
	try {
		readSkillsDir(nodePath.join(cwd, ".agents", "skills"), cwd, skills, false);
	} catch {}
...
async function loadAgentsBundle() {
	...
	try {
		const mod = await import("./_virtual_agents-bundle-Gm9rxJme.js");
		if (mod && mod.default) {
			cached = mod.default;
			return cached;
		}
