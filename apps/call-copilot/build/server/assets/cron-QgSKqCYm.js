import { a as __toCommonJS, i as __require, n as __esmMin, r as __exportAll, t as __commonJSMin } from "./chunk-D3zDcpJC.js";
...
var luxon_exports = /* @__PURE__ */ __exportAll({
	DateTime: () => DateTime,
	Duration: () => Duration,
...
var require_CronDate = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var luxon_1 = (init_luxon(), __toCommonJS(luxon_exports));
	var TimeUnit;
	(function(TimeUnit) {
...
//#endregion
//#region ../../node_modules/.pnpm/@agent-native+core@0.14.8_d8cd0b9ea502c97d2f2b6c28b2fc9a81/node_modules/@agent-native/core/dist/jobs/cron.js
...
var ALIAS_MAP = { "@midnight": "0 0 * * *" };
function normalize(cronExpr) {
	return ALIAS_MAP[cronExpr.trim().toLowerCase()] ?? cronExpr;
}
/**
* Compute the next occurrence of a cron expression after the given date.
*/
function nextOccurrence(cronExpr, after) {
	return import_dist.CronExpressionParser.parse(normalize(cronExpr), { currentDate: after ?? /* @__PURE__ */ new Date() }).next().toDate();
}
/**
...
function isValidCron(cronExpr) {
	try {
		import_dist.CronExpressionParser.parse(normalize(cronExpr));
		return true;
	} catch {
		return false;
	}
...
function describeCron(cronExpr) {
	...
	if (minMatch && hour === "*" && dayOfMonth === "*" && month === "*" && dayOfWeek === "*") return `Every ${minMatch[1]} minutes`;
	if (minute !== "*" && hour === "*" && dayOfMonth === "*" && month === "*" && dayOfWeek === "*") return `Every hour at :${minute.padStart(2, "0")}`;
	const formatTime = (h, m) => {
		if (h === "*") return "";
		return h.split(",").map((hh) => {
			const hr = parseInt(hh, 10);
			const ampm = hr >= 12 ? "PM" : "AM";
			return `${hr === 0 ? 12 : hr > 12 ? hr - 12 : hr}${m === "0" || m === "00" ? "" : `:${m.padStart(2, "0")}`} ${ampm}`;
		}).join(" and ");
	};
	...
	if (dayOfMonth === "*" && month === "*" && dayOfWeek === "*" && time) return `Every day at ${time}`;
	if (dayOfMonth === "*" && month === "*" && (dayOfWeek === "1-5" || dayOfWeek === "MON-FRI") && time) return `Every weekday at ${time}`;
	if (dayOfMonth === "*" && month === "*" && dayOfWeek !== "*" && time) return `Every ${dayOfWeek.split(",").map((d) => dayNames[d] || d).join(", ")} at ${time}`;
	if (dayOfMonth !== "*" && month === "*" && dayOfWeek === "*" && time) return `On day ${dayOfMonth} of every month at ${time}`;
	return cronExpr;
}
//#endregion
export { describeCron, isValidCron, nextOccurrence };
