# My Platform — Agent-Native Workspace
...
**Contents:** [Layout](#layout) · [Three-layer inheritance](#three-layer-inheritance) · [Getting started](#getting-started) · [Workspace org identity](#workspace-org-identity) · [Adding a new app](#adding-a-new-app) · [Editing shared behavior](#editing-shared-behavior) · [Call Copilot](#call-copilot)
...
my-platform/
├── packages/
│   └── shared/               # @my-platform/shared — optional shared code
...
└── apps/
    ├── call-copilot/         # Live transcription, KB, call analysis (see below)
    ├── dispatch/
    ├── forms/
    ├── mail/
    ├── design/
...
the shared package (`@my-platform/shared`).
...
Dispatch app selected, and each app at its own path (for example `/call-copilot`, `/forms`). It
watches `apps/`, so newly-created apps are detected and started without
restarting `pnpm dev`.

**Call Copilot** needs app-local secrets in `apps/call-copilot/.env.local` (e.g. `DEEPGRAM_API_KEY`, `BUILDER_PRIVATE_KEY`) — see [Call Copilot](#call-copilot) below.
...
depends on `@my-platform/shared`. Edit only the routes you care about;
...
Source tree: `apps/call-copilot/`. In multi-app workspaces the UI is typically mounted at **`/call-copilot`**; app-local routes use `/` inside the app shell.
...
- **Transcript sources** — Saved transcripts, paste, or `.txt` / `.vtt` import (`parseTranscriptUploadText` in `app/lib/transcript-import.ts`).
...
- [`apps/call-copilot/AGENTS.md`](./apps/call-copilot/AGENTS.md) — Call Copilot agent resources, application state keys, action cheat sheet.
- [`apps/call-copilot/DEVELOPING.md`](./apps/call-copilot/DEVELOPING.md) — Call Copilot framework patterns (routes, APIs, plugins, scripts).
- [`AGENTS.md`](./AGENTS.md) (repo root) — Monorepo orchestration and org repair.
